...
void solve()
{
    ...
    get(c);
    v2ll(dp, n, 4, inf);
    //dp[i][j] is the min cost to get j displays with increasing font size
    //using first i displays
    dp[0][1] = c[0];
    fk(i, 1, n)
    {
        fo(j, i)
        {
            ...
            if (s[j] < s[i])
            {
                dp[i][2] = min(dp[i][2], min(dp[j][1] + c[i], dp[j][2]));
                dp[i][3] = min(dp[i][3], min(dp[j][2] + c[i], dp[j][3]));
            }
        }
    }
    ll ans = inf;
